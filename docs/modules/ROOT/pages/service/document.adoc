...
[source,text]
----
POST /internal/dookug/document/generate/inline?responseContentGzipped={true|false}
----
...
In the response, we receive the generated document (Gzipped if needed) and the filename in the HTTP header.
...
[source,text]
----
POST /internal/dookug/document/generate/inline/multipart?responseContentGzipped={true|false}
----
...
[source,text]
----
POST /internal/dookug/document/generate/storedTemplate?responseContentGzipped={true|false}
----
...
[source,text]
----
GET /internal/dookug/document/content/{documentId}?responseContentGzipped={true|false}
----
...
In the response - for an existing document identifier - the generated document is returned (Gzipped if needed), and the file name is included in the HTTP headers.
...
[source,text]
----
GET /test/dookug/document/storedTemplate/metadata
----

Returns template metadata list from the TEMPLATE table according to the specified filtering, sorting url query parameters.
...
* validityStart
** inclusive
** input is set to start of the day in the query
** format: yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ
* validityEnd
** exclusive
** input is set to start of next day in the query
** format: yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ

Ordering (ASC/DESC) can be specified for each sorting parameter individually:

* name
* lastUpdatedAt → default DESC
* description
* language
* validityStart
* validityEnd

XSLT (generator_engine=saxon) templates are grouped in the response based on their name and validity.
...
[source,text]
----
POST /test/dookug/document/generate/inline/multipart
----

Generates a document based on the provided multipart form data.

[options="header",cols="1,3"]
|===
|Input Part  |Description
|TEMPLATE (file)  |Required, 1 file: the template file to be used for document generation
|SUBTEMPLATE (file) |Optional, can be multiple: template files to be used as template parts
|TEMPLATE_LANGUAGE (string) |Optional, but required if the template is xslt: the language of the template
|PARAMETERS_TEMPLATE_ENGINE (file) |Optional, 1 file, accepted extension: json
|PARAMETERS_GENERATOR_ENGINE (file) |Optional, 1 file, accepted extension: xml
|RESPONSE_CONTENT_GZIPPED (string) |Optional, if true, the response content will be GZIP compressed
|===
...
[source,text]
----
POST /test/dookug/document/storedTemplateGroup
----

Insert new template group to the TEMPLATE, TEMPLATE_PART, TEMPLATE_TEMPLATE_PART, TEMPLATE_PART_CONTENT tables.

[options="header",cols="1,3"]
|===
|Input Part  |Description
|TEMPLATE (file)  |Required, at least 1 file: the template parts. Accepted extensions: html, txt, xslt. All files must have the same extension.
|TEMPLATE_FILE_IDS (text) |Required, every file must have an identifier.
|TEMPLATE_DATA (json) |Required, `CreateTemplateGroupRequest` in json format. Must be schema valid.
|===

Example of `CreateTemplateGroupRequest`:
[source, json]
----
{
  "context": {
