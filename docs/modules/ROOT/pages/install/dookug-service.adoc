...
[source,properties]
----
OTEL_SDK_DISABLED=false #optional, default is false
OPENTELEMETRY_SERVICE_NAME=dookug-document-service
OTEL_SERVICE_NAME=dookug-document-service <1>
OPENTELEMETRY_SAMPLER_TYPE: ratio
OPENTELEMETRY_SAMPLER_RATIO: 0.01
...
----
<1> If the OPENTELEMETRY_SERVICE_NAME variable wouldn't work
...
|OPENTELEMETRY_SAMPLER_TYPE| ratio| on (full sampling) / off (no sampling) / ratio (default: ratio)
|OPENTELEMETRY_SAMPLER_RATIO| 0.01| needed if OPENTELEMETRY_SAMPLER_TYPE=ratio (default: 0.01)
